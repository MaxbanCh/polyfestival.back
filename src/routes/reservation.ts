...
import { addReservation, updateReservation, listReservations, listReservationsByFestival, listReservationsByReservant } from '../reservation/reservation.ts';
...
reservationRouter.get('/', async (req, res) => {
    if (req.query.festivalId) {
        ...
        const reservations: Reservation[] = await listReservationsByFestival(festivalId);
        res.json(reservations);
        return;
    }
    if (req.query.reservantId) {
        const reservantId = parseInt(req.query.reservantId as string, 10);
        const reservations: Reservation[] = await listReservationsByReservant(reservantId);
        res.json(reservations);
        return;
    }
    else {
...
reservationRouter.post('/:id', async (req, res) => {
    ...
    const updatedReservation: Reservation | null = await updateReservation(req.body);
