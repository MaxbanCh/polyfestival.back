...
async function listMapZone(): Promise<MapZone[]> {
  const res = await pool.query(
    'SELECT * FROM map_zones ORDER BY festival_id, name;',
  );
...
async function listMapZoneByFestival(festivalId: number): Promise<MapZone[]> {
  ...
  const res = await pool.query(
    'SELECT * FROM map_zones WHERE festival_id = $1 ORDER BY name;',
    [festivalId],
  );
  return res.rows.map((row) => ({
    id: row.id,
...
async function listMapZoneByTarifZone(tarifZoneId: number): Promise<MapZone[]> {
  ...
  const res = await pool.query(
    'SELECT * FROM map_zones WHERE tariffzoneid = $1 ORDER BY name;',
    [tarifZoneId],
  );
  return res.rows.map((row) => ({
    id: row.id,
