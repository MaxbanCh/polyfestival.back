...
async function listTarifZone(): Promise<TarifZone[]> {
    const res = await pool.query('SELECT * FROM tarif_zones ORDER BY festival_id, name;');
...
async function addTarifZone(tarifZone: Omit<TarifZone, "id">): Promise<TarifZone> {
    ...
    const res = await pool.query(
        ...
        [tarifZone.name, tarifZone.festivalId, tarifZone.nbtable, tarifZone.tableprice, tarifZone.pricem2, tarifZone.availableTables]
    );
    const row = res.rows[0];
    return {
        id: row.id,
...
async function updateTarifZone(tarifZone: TarifZone): Promise<TarifZone | null> {
    const res = await pool.query(
        `UPDATE tarif_zones
         SET name = $1, festival_id = $2, nbtable = $3, tableprice = $4, pricem2 = $5, available_tables = $6
         WHERE id = $7 RETURNING *`,
        [tarifZone.name, tarifZone.festivalId, tarifZone.nbtable, tarifZone.tableprice, tarifZone.pricem2, tarifZone.availableTables, tarifZone.id]
    );
    if (res.rowCount === 0) {
        return null;
    }
