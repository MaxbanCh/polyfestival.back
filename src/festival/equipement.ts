...
async function listEquipment(): Promise<Equipment[]> {
    const res = await pool.query('SELECT * FROM equipment ORDER BY festival_id, kind;');
...
async function addEquipment(equipment: Omit<Equipment, "id">): Promise<Equipment> {
    ...
    const res = await pool.query(
        ...
        [equipment.festivalId, equipment.kind, equipment.unitPrice, equipment.quantity]
    );
    const row = res.rows[0];
    return {
        id: row.id,
...
async function updateEquipment(equipment: Equipment): Promise<Equipment | null> {
    const res = await pool.query(
        `UPDATE equipment
         SET festival_id = $1, kind = $2, unit_price = $3, quantity = $4
         WHERE id = $5 RETURNING *`,
        [equipment.festivalId, equipment.kind, equipment.unitPrice, equipment.quantity, equipment.id]
    );
    if (res.rowCount === 0) {
        return null;
    }
