...
async function addTable(festivalId: number, type: TableType, quantityUsedTable: number, quantityMaxTable: number): Promise<Table> {
...
async function getTablesforFestival(festivalId: number) : Promise<Table[]> {
    ...
    const res = await pool.query('SELECT * FROM tables WHERE festival_id = $1 ORDER BY name;', [festivalId]);
...
async function modifyTable(table: Table): Promise<Table | null> {
    const res = await pool.query(
        ...
        [table.festivalId, table.type, table.quantityUsedTable, table.quantityMaxTable, table.id]
    );
